...
[TiDB](https://en.pingcap.com/) is an open-source distributed HTAP database compatible with the MySQL protocol. It features horizontal scalability, strong consistency, and high availability.
...
- You need to create a workspace, a project, and several user accounts (`admin`, `ws-admin`, `project-regular`) for this tutorial. The account `ws-admin` must be granted the role of `workspace-admin` in the workspace, and the account `project-regular` must be invited to the project with the role of `operator`. If they are not ready, refer to [Create Workspaces, Projects, Accounts and Roles](../../../quick-start/create-workspace-and-project/).

## Hands-on Lab

### Step 1: Install TiDB Operator CRD

1. Log in to KubeSphere Web console as `admin`, use the **Kubectl** from the **Toolbox** in the bottom right corner to execute the following command to install TiDB Operator CRD:
...
### Step 3: Deploy the TiDB Operator
...
4. Select the **Chart Files** tab, you can view the configuration from the console directly or download the default `values.yaml` file by clicking the icon on the upper right corner. Under **Versions**, select a version number from the drop-down list and click **Deploy**.
...
8. Go to **Workloads**, you can see two deployments created for TiDB Operator.
...
The process of deploying a TiDB cluster is similar to deploying the TiDB Operator.

1. Go to **Applications** under **Application Workloads** and click **Deploy New Application** again and then select **From App Templates**.
...
5. Some TiDB components require [persistent volumes](../../../cluster-administration/persistent-volume-and-storage-class/). You can run the following command to view your storage classes.
   ...
   In this regard, the following storage classes will be deployed automatically by default if you install KubeSphere through QingCloud Kubernetes Engine (QKE).
   ...
   The QingCloud CSI plugin implements an interface between [CSI](https://github.com/container-storage-interface/)-enabled Container Orchestrator (CO) and the storage of QingCloud. For more information about QingCloud CSI, see the [GitHub repository](https://github.com/yunify/qingcloud-csi).

   {{</ notice >}}

6. On the **App Config** page, change the default value of the field `storageClassName` from `local-storage` to `csi-qingcloud`. In the downloaded file, you can replace all the values directly and copy and paste the modified local file into the `values.yaml` file on the console.
...
1. Go to **Workloads** under **Application Workloads**, you can see all TiDB cluster Deployments are up and running.
...
2. Switch to **StatefulSets** tab, you can see TiDB, TiKV and PD are up and running.
...
3. Click a single StatefulSet to go to its details page. You can see the metrics in line charts over a period of time under the **Monitoring** tab.
...
4. Go to **Pods** under **Application Workloads**, you can see the TiDB cluster contains two TiDB Pods, three TiKV Pods, and three PD Pods.

   ![tidb-pod-list](/images/docs/appstore/external-apps/deploy-tidb-operator-and-cluster/tidb-pod-list.PNG)

5. Go to **Volumes** under **Storage**, you can see TiKV and PD are using persistent volumes.

   ![tidb-storage-usage](/images/docs/appstore/external-apps/deploy-tidb-operator-and-cluster/tidb-storage-usage.PNG)

6. Volume usage is also monitored. Click a volume item to go to the details page. Here is an example of TiKV:
...
1. Go to **Services** under **Application Workloads**, you can see detailed information of all services. As the service type is set to `NodePort` by default, you can access it through the Node IP address outside the cluster.
