...
* 成员集群移除后，{ks_product-en}将无法管理该集群。集群中原有的 Kubernetes 资源将不会被删除。
...
集群被移除后，集群中原有的多集群配置数据不会被自动清除，卸载{ks_product-en}或删除关联资源时可能会导致用户数据丢失。请务必在被移除的集群上执行link:#_清理多集群配置数据[清理多集群配置数据]步骤。
