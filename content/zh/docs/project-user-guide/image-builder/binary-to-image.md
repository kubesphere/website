...
Binary-to-Image (B2I) 是一个工具箱和工作流，用于从二进制可执行文件（例如 Jar、War 和二进制包）构建可复制的容器镜像。更确切地说，您可以上传一个制品 (Artifact) 并指定一个目标仓库，例如 Docker Hub 或者 Harbor，用于推送镜像。如果所有配置都成功运行，会推送您的镜像至目标仓库，并且如果您在工作流中创建服务 (Service)，也会自动部署应用程序至 Kubernetes。
...
4. 在**容器设置**页面，下拉至**服务设置**，为容器设置访问策略。**协议**选择 **HTTP**，自定义名称（例如 `http-port`），**容器端口**和**服务端口**都输入 `8080`。点击**下一步**继续。
   ...
   有关如何在**容器设置**页面设置其它参数的更多信息，请参见[容器镜像设置](../../../project-user-guide/application-workloads/container-image-settings/)。
...
2. 点击该镜像前往其详情页面。在**任务记录**下，点击记录右侧的箭头图标查看构建日志。如果所有运行都正常，您可以在日志末尾看到 `Build completed successfully`。
